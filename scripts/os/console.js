       ...
       while (_KernelInputQueue.getSize() > 0)
       {
			...
			else if (chr == String.fromCharCode(8))  // Backspace
			{
				if(this.buffer !== "")
				{
					...
					var lastChr = this.buffer.substring(this.buffer.length, this.buffer.length - 1);
				...
				while(keepSplitting)
				{
					...
					if(_DrawingContext.measureText(this.CurrentFont, this.CurrentFontSize, splitLine) < _Canvas.width - 4)
					{
						...
						_DrawingContext.drawText(this.CurrentFont, this.CurrentFontSize,
							this.CurrentXPosition, this.CurrentYPosition, splitLine);
						// Move the current X position.
						var offset = _DrawingContext.measureText(this.CurrentFont, this.CurrentFontSize, splitLine);
